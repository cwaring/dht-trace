...
function Edge ({ layout, first, second, options }) {
  ...
  const d = `M 0 ${-halfDistance} q ${controlPoint[0]} ${controlPoint[1]}, 0 ${distance}`
...
export default function Graph ({ peers, paths, options = {} }) {
  ...
  const nodes = peers.map((peer) => {
    return <Node key={peer.b58} layout={layout} peer={peer} options={options} />
  })
  ...
  const edges = paths.map(path => {
    ...
    return <Edge key={key} layout={layout} first={first} second={second} options={options} />
  })
